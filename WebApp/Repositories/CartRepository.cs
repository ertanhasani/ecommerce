...
public class CartRepository : ICartRepository
{
    ...
    public List<OrderDetails> GetDigitFileList(string userId)
    {
        return _context.OrderDetails.Include(p => p.Order).Include(o => o.Product).Where(p => p.Order.UserId == userId && p.Order.Payed == true && p.IsDeleted == false).ToList();
    }
    ...
    {
        return _context.OrderDetails.Include(p => p.Product).Include(o => o.Order).FirstOrDefault(p => p.Id == orderDetailsId && p.IsDeleted == false);
    }
